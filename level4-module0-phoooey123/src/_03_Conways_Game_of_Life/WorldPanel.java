...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
	...
	public int getLivingNeighbors(int x, int y){
		...
		if(y != cells.length && cells[x][y+1].isAlive) {
			r++;
		}
		if(x != 0 && cells[x-1][y].isAlive) {
			r++;
		}
		if(x != cells.length && cells[x+1][y].isAlive) {
			r++;
		}
		if(x != 0 && y != 0 && cells[x-1][y-1].isAlive) {
			r++;
		}
		if(x != cells.length && y != 0 && cells[x+1][y-1].isAlive) {
			r++;
		}
		if(x != 0 && y!= cells.length &&cells[x-1][y+1].isAlive) {
			r++;
		}
		if(x != cells.length && y!= cells.length && cells[x+1][y+1].isAlive) {
			r++;
		}
		return r;
	}
